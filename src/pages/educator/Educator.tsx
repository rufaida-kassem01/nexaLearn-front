...
const Educator = () => {
  ...
  const [applicationStatus, setApplicationStatus] = useState<InstructorApplication | null>(null);
  ...
  const handleApplyInstructor = async () => {
    ...
    try {
      ...
      const message = (err as { response?: { data?: { message?: string } } })?.response?.data?.message || (err instanceof Error ? err.message : "Failed to submit application");
      setError(message);
