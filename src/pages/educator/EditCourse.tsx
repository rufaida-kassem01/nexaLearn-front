...
const EditCourse = () => {
  ...
  useEffect(() => {
    ...
    const fetchCourse = async () => {
      ...
      try {
        ...
        const reconstructedChapters: Chapter[] = (course.modules || [])
          .slice()
          .sort(
            (a, b) =>
              ((a as unknown as Record<string, number>).orderIndex ?? (a as unknown as Record<string, number>).order ?? 0) -
              ((b as unknown as Record<string, number>).orderIndex ?? (b as unknown as Record<string, number>).order ?? 0),
          )
          .map((module) => ({
            ...
            chapterContent: (module.lessons || [])
              ...
              .map((lesson) => ({
                ...
                lectureContentType: lesson.contentType
                  ...
                  : "video",
                isPreviewFree: (lesson as unknown as Record<string, boolean>).isPreviewFree || lesson.isPreview || false,
                lectureOrder: 0,
              })),
            chapterOrder: (module as unknown as Record<string, number>).orderIndex ?? (module as unknown as Record<string, number>).order ?? 0,
            collapsed: false,
          }));
  ...
  const isPersistedId = (id: string) => /^\d+$/.test(String(id));

  const handleMoveModule = async (chapterIndex: number, direction: "up" | "down") => {
    ...
    try {
      ...
      addToast(err instanceof Error ? err.message : "Failed to reorder modules.", "error");
    }
  };

  const handleMoveLesson = async (chapterId: string, lectureIndex: number, direction: "up" | "down") => {
    ...
    try {
      ...
      setChapters(previousChapters);
      addToast(err instanceof Error ? err.message : "Failed to reorder lessons.", "error");
    }
  };
  ...
  return (
    <div className="h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0">
      <form
        ...
        className="flex flex-col gap-4 max-w-md w-full text-gray-500 dark:text-gray-400"
      >
        <h1 className="text-2xl font-semibold text-gray-800 dark:text-gray-100">Edit Course</h1>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            className="outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500 dark:border-gray-600 dark:bg-slate-700 dark:text-gray-200 dark:placeholder-gray-500"
            required
          />
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          {categories.length > 0 ? (
            <select
              ...
              className="outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500 dark:border-gray-600 bg-white dark:bg-slate-700 dark:text-gray-200"
              required
            >
              {categories.map((cat) => (
        ...
        <div className="flex items-center justify-between flex-wrap">
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              step="0.01"
              disabled={isFree}
              className="outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500 dark:border-gray-600 dark:bg-slate-700 dark:text-gray-200 disabled:opacity-40"
              required={!isFree}
            />
          </div>
        ...
        <div className="flex flex-col gap-1">
          <p>Discount % <span className="text-xs text-gray-400 dark:text-gray-500">(coming soon)</span></p>
          <input
        ...
        <div>
          {chapters.map((chapter, chapterIndex) => (
            <div key={chapter.chapterId} className="bg-white dark:bg-slate-800 border dark:border-gray-600 rounded-lg mb-4">
              ...
              {!chapter.collapsed && (
                <div className="p-4">
                  ...
                  <div
                    className="inline-flex bg-gray-100 dark:bg-gray-700 p-2 rounded cursor-pointer mt-2"
          ...
          <div
            className="flex justify-center items-center bg-blue-100 dark:bg-blue-900/40 p-2 rounded-lg cursor-pointer"
          ...
          {showPopup && (
            <div className="fixed inset-0 flex items-center justify-center bg-black/30 backdrop-blur-sm">
              <div className="bg-white dark:bg-slate-800 text-gray-700 dark:text-gray-200 p-4 rounded relative w-full max-w-80">
                ...
                <div className="mb-2">
                  ...
                  <input
                    type="text"
                    className="mt-1 block w-full border dark:border-gray-600 rounded py-1 px-2 dark:bg-slate-700 dark:text-gray-200"
                ...
                <div className="mb-2">
                  <p>Content Type</p>
                  <select
                    className="mt-1 block w-full border dark:border-gray-600 rounded py-1 px-2 bg-white dark:bg-slate-700 dark:text-gray-200"
        ...
        <div className="flex gap-3 my-4">
          <button
            ...
            className="bg-black dark:bg-slate-600 text-white w-max py-2.5 px-8 rounded disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <button
            type="button"
            onClick={() => navigate("/educator/my-courses")}
            className="bg-gray-300 dark:bg-gray-600 dark:text-gray-200 text-gray-700 w-max py-2.5 px-8 rounded hover:bg-gray-400 dark:hover:bg-gray-500 transition"
          >
            Cancel
          </button>
        </div>
