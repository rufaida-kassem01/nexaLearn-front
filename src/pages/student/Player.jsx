...
const RateCourse = ({ courseId, accessToken }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || "Couldn't submit your review. Please try again.");
...
const ChatPanel = ({ lessonId, accessToken, onClose }) => {
...
const QuizWidget = ({ lesson, accessToken, onClose }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const isEnrolled = enrollmentResp?.data?.enrolled ?? false;
        if (!isEnrolled) {
          navigate(`/course/${courseId}`, { replace: true });
          return;
        }

        const modules = (courseDetail.modules || []).map((mod, mi) => ({
          id: mod.id,
          title: mod.title,
