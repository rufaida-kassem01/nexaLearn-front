...
interface AppWithUser extends InstructorApplication {
  user?: AppUser;
  submittedAt?: string;
  reviewedAt?: string;
...
const RejectModal = ({ open, loading, onClose, onConfirm }: RejectModalProps) => {
...
const ApplicationCard = ({ app, onApprove, onReject }: ApplicationCardProps) => {
...
const InstructorApplications = () => {
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      toast.error(
        (err as { response?: { data?: { message?: string } } })?.response?.data?.message || "Failed to approve application",
      );
    } finally {
  ...
  const handleReject = async (reason: string) => {
    ...
    try {
      ...
      toast.error(
        (err as { response?: { data?: { message?: string } } })?.response?.data?.message || "Failed to reject application",
      );
    } finally {
      setActionLoading(false);
    }
