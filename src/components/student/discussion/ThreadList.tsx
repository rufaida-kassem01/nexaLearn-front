...
const ThreadList = ({ lessonId, onClose }: ThreadListProps) => {
  const { user } = useAuth();
  ...
  const fetchThreads = useCallback(async (cursor: string | null) => {
    setLoading(true);
    setError("");
    try {
      ...
      setError(apiErr?.response?.data?.message || apiErr.message || "Failed to load threads.");
    } finally {
      setLoading(false);
    }
  ...
  const handleCreated = (thread: ThreadItem) => {
    setThreads((prev) => [thread, ...prev]);
    setShowCreate(false);
  };

  const handleDelete = async (thread: ThreadItem) => {
    try {
  ...
  const handlePinToggle = async (thread: ThreadItem) => {
    try {
      if (thread.isPinned) {
      ...
      const apiErr = err as { response?: { data?: { message?: string } }; message?: string };
      const msg = apiErr?.response?.data?.message || apiErr.message || "Failed to update pin.";
      toast.error(msg);
    }
  };
