...
type UploadStatus = "idle" | "uploading" | "finalizing" | "completed" | "error";

const VideoUploader = ({ courseId, lessonId, onComplete }: VideoUploaderProps) => {
  ...
  const isActive: boolean = status === "uploading" || status === "finalizing";
  ...
  const startUpload = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        addToast(
          apiErr?.response?.data?.message || apiErr.message || "Failed to start upload.",
          "error"
        );
      }
    },
  ...
  const pollAssetStatus = async (assetId: string) => {
    ...
    const poll = async () => {
      attempts++;
      try {
        const asset: VideoAsset = await mediaService.getVideoAsset(assetId);
