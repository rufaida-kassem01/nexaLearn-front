...
const QuizBuilder = ({ courseId, lessonId, lessonTitle, onClose }) => {
  ...
  const noRealIds = !courseId || !lessonId;
  ...
  const fetchQuiz = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await assessmentService.instructorGetQuiz(courseId, lessonId);
      setQuiz(data);
      setSettings({
        title: data.title || "",
  ...
  const handleCreateOrUpdateQuiz = async () => {
    ...
    try {
      const payload = {
        ...
        timeLimitMins: settings.timeLimitMins ? parseInt(settings.timeLimitMins) : undefined,
        maxAttempts: settings.maxAttempts ? parseInt(settings.maxAttempts) : undefined,
      };

      const result = await assessmentService.createQuiz(courseId, lessonId, payload);
      setQuiz(result);
  ...
  const handleAddQuestion = async (qData) => {
    try {
      const result = await assessmentService.addQuestion(courseId, lessonId, qData);
      setQuiz(result);
      ...
      addToast(
        err?.response?.data?.message || err.message || "Failed to add question.",
        "error",
      );
    }
  };
  ...
  const handleRemoveQuestion = async (questionId) => {
    try {
      const result = await assessmentService.removeQuestion(courseId, lessonId, questionId);
      setQuiz(result);
      addToast("Question removed.", "success");
    } catch (err) {
      addToast(
        err?.response?.data?.message || err.message || "Failed to remove question.",
        "error",
      );
    }
  };
  ...
  const handlePublish = async () => {
    setPublishing(true);
    try {
      const result = await assessmentService.publishQuiz(courseId, lessonId);
      setQuiz((prev) => ({ ...prev, ...result }));
      addToast("Quiz published!", "success");
    } catch (err) {
      addToast(
        err?.response?.data?.message || err.message || "Failed to publish quiz.",
        "error",
      );
    } finally {
      setPublishing(false);
  ...
  const handleUnpublish = async () => {
    setPublishing(true);
    try {
      const result = await assessmentService.unpublishQuiz(courseId, lessonId);
      setQuiz((prev) => ({ ...prev, ...result }));
      addToast("Quiz unpublished.", "success");
    } catch (err) {
      addToast(
        err?.response?.data?.message || err.message || "Failed to unpublish quiz.",
        "error",
      );
    } finally {
      setPublishing(false);
  ...
  const questions = (quiz?.questions || []).slice().sort((a, b) => (a.orderIndex || 0) - (b.orderIndex || 0));
