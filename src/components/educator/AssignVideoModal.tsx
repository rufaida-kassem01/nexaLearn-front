...
interface AssignVideoModalProps {
  ...
  chapters: Chapter[];
  onClose: () => void;
  onAssigned: (asset: VideoAsset) => void;
}

const AssignVideoModal = ({ asset, chapters, onClose, onAssigned }: AssignVideoModalProps) => {
  ...
  const handleAssign = async () => {
    ...
    setAssigning(true);
    try {
      const updatedAsset: VideoAsset = await mediaService.assignVideoToLesson(
        asset.id,
        selectedLessonId
      );
  ...
  const isNumericId = (id: string): boolean => /^\d+$/.test(String(id));
