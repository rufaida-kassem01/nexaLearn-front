...
interface OptionWithOrder extends QuestionOption {
  orderIndex?: number;
  optionText?: string;
}
...
const QuizPreview = ({ quiz, onClose }: QuizPreviewProps) => {
  ...
  const quizData = quiz as QuestionWithOrder;
  const questions: QuestionWithOrder[] = quiz.questions || [];
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl mx-4 relative">
        ...
        <div className="mb-6 border-b pb-4">
          ...
          <div className="flex gap-4 mt-2 text-xs text-gray-400">
            ...
            {quizData.totalMarks != null && <span>Total: {quizData.totalMarks} pts</span>}
            {quizData.timeLimitMins && <span>Time: {quizData.timeLimitMins} min</span>}
            {quizData.maxAttempts && <span>Max attempts: {quizData.maxAttempts}</span>}
          </div>
        </div>
                ...
                <div key={q.id} className="border rounded-lg p-4">
                  ...
                  {q.type === "SHORT_ANSWER" ? (
                    ...
                    <div className="space-y-1.5">
                      {(q.options || [])
                        .slice()
                        .sort((a: OptionWithOrder, b: OptionWithOrder) => (a.orderIndex || 0) - (b.orderIndex || 0))
                        .map((opt: OptionWithOrder) => (
                          <label
                            key={opt.id}
