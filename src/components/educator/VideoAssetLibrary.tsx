...
interface Chapter {
  chapterId: string;
  chapterTitle: string;
  chapterContent: { lectureId: string; lectureTitle: string }[];
}

interface VideoAssetLibraryProps {
  courseId: string;
  chapters: Chapter[];
  onAssetAssigned?: (asset: VideoAsset) => void;
}

const VideoAssetLibrary = ({ courseId, chapters, onAssetAssigned }: VideoAssetLibraryProps) => {
  const { addToast } = useToast();
  const [assets, setAssets] = useState<VideoAsset[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [showUploader, setShowUploader] = useState<boolean>(false);
  const [assignTarget, setAssignTarget] = useState<VideoAsset | null>(null);
  ...
  const handleUploadComplete = (asset: VideoAsset) => {
    setShowUploader(false);
    setAssets((prev) => [asset, ...prev]);
    addToast("Video uploaded successfully! Transcoding in progress.", "success");
  };

  const handleAssign = (asset: VideoAsset) => {
  ...
  const handleAssigned = (updatedAsset: VideoAsset) => {
    setAssets((prev) =>
  ...
  const formatDuration = (secs: number | undefined): string | null => {
    if (!secs) return null;
