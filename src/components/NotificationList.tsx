...
const NotificationList = ({ onClose }: NotificationListProps) => {
  ...
  const [items, setItems] = useState<NotificationWithData[]>([]);
  const [nextCursor, setNextCursor] = useState<string | null>(null);
  const [loading, setLoading] = useState<boolean>(false);

  const fetch = useCallback(async (cursor: string | null) => {
    setLoading(true);
    try {
      const resp = await listNotifications({ cursor: cursor ?? undefined, limit: 20 });
  ...
  const handleClick = async (notification: NotificationWithData) => {
    ...
    if (navData.courseId) {
      navigate(`/course/${navData.courseId}`);
    } else if (navData.lessonId) {
      navigate(`/player/${navData.courseId || ""}/${navData.lessonId}`);
    }
    onClose?.();
  };
